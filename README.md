# Kredits Onion Blockchain Explorer
 ...
 **Blocks and transactions**
 ...
 - shows public components of Kredits addresses,
 - decoding which outputs and mixins belong to the given Kredits address and viewkey,
 - can prove that you send Kredits to someone,
 - detailed information about mixins, such as, mixins' age, timescale, mixin of mixins,
 - shows number of amount output indices,
 - support Kredits testnet, stagenet network,
...
Current development branch:
 ...
 - https://github.com/doy-lee/onion-kredits-blockchain-explorer/tree/kredits



## Compilation on Ubuntu 16.04/18.04

##### Compile latest Kredits master version

Download and compile recent Kredits into your home folder:

```bash
# first install Kredits dependecines
sudo apt update
...
Once the Kredits is compiles, the explorer can be downloaded and compiled
as follows:

```bash
# go to home folder if still in ~/kredits
cd ~

# download the source code
git clone https://github.com/kredits-project/kredits-onion-blockchain-explorer.git

# enter the downloaded sourced code folder
cd kredits-onion-blockchain-explorer
...
# alternatively can use: cmake -DKREDITS_DIR=/path/to/kredits ..
# if kredits is not in ~/kredits
#
...
When compilation finishes executable `xkrblocks` should be created. Before running
...
By default it will look for blockchain in its default location i.e., `~/.kredits/lmdb`.
...
```
xkrblocks, Onion Kredits Blockchain Explorer:
  ...
  --enable-emission-monitor [=arg(=1)] (=0)
                                        enable Kredits total emission monitoring
                                        thread
...
alias xkrblocksmainnet='~/kredits-onion-blockchain-explorer/build/xkrblocks    --port 8081 --testnet-url "http://139.162.32.245:8082" --enable-pusher --enable-emission-monitor'

# for testnet explorer
alias xkrblockstestnet='~/kredits-onion-blockchain-explorer/build/xkrblocks -t --port 8082 --mainnet-url "http://139.162.32.245:8081" --enable-pusher --enable-emission-monitor'
```

These are aliases similar to those used for http://139.162.32.245:8081/ and http://139.162.32.245:8082/, respectively.

## Enable Kredits emission 

Obtaining current Kredits emission amount is not straight forward. Thus, by default it is 
disabled. To enable it use `--enable-emission-monitor` flag, e.g., 


```bash
xkrblocks --enable-emission-monitor 
```
...
Every 10000 blocks, the thread will save current emission in a file, by default, 
 in `~/.kredits/lmdb/emission_amount.txt`. For testnet or stagenet networks, 
 it is `~/.kredits/testnet/lmdb/emission_amount.txt` or `~/.kredits/stagenet/lmdb/emission_amount.txt`. This file is used so that we don't
 need to rescan entire blockchain whenever the explorer is restarted. When the 
 explorer restarts, the thread will first check if `~/.kredits/lmdb/emission_amount.txt`
...
```
Kredits emission (fees) is 14485540.430 (52545.373) as of 1313448 block
```

The values given, can be checked using Kredits daemon's  `print_coinbase_tx_sum` command. 
...
Having the `crt` and `key` files, run `xkrblocks` in the following way:

```bash
./xkrblocks --ssl-crt-file=/tmp/server.crt --ssl-key-file=/tmp/server.key 
```
...
We use recipient's address (i.e. not our address from which we sent xkr to recipient).
...
```json
{
  "data": {
    ...
    "git_branch_name": "update_to_current_kredits",
    "last_git_commit_date": "2017-07-25",
    "last_git_commit_hash": "a549f25",
    "kredits_version_full": "0.10.3.1-ab594cfe"
  },
  "status": "success"
}
```
...
Return raw json block data, as represented in Kredits.
...
Return raw json tx data, as represented in Kredits.
