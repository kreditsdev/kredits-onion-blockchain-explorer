...
main(int ac, const char* av[])
{
    ...
    xkreg::CmdLineOptions opts {ac, av};
    ...
    if (!xkreg::get_blockchain_path(bc_path_opt, blockchain_path, nettype)) {
    ...
    xkreg::MicroCore mcore;
    cryptonote::Blockchain* core_storage;

    // initialize mcore and core_storage
    if (!xkreg::init_blockchain(blockchain_path.string(),
    ...
    if (enable_emission_monitor == true)
    {
        ...
        xkreg::CurrentBlockchainStatus::blockchain_path
                = blockchain_path;
        xkreg::CurrentBlockchainStatus::nettype
                = nettype;
        xkreg::CurrentBlockchainStatus::daemon_url
                = daemon_url;
        xkreg::CurrentBlockchainStatus::set_blockchain_variables(
                &mcore, core_storage);
        ...
        xkreg::CurrentBlockchainStatus::start_monitor_blockchain_thread();
    }


    xkreg::MempoolStatus::blockchain_path
            = blockchain_path;
    xkreg::MempoolStatus::nettype
            = nettype;
    xkreg::MempoolStatus::daemon_url
            = daemon_url;
    xkreg::MempoolStatus::set_blockchain_variables(
            &mcore, core_storage);

    xkreg::MempoolStatus::network_info initial_info;
    ...
    xkreg::MempoolStatus::current_network_info = initial_info;
    ...
    // by tx searching threads that are launched for each user independently,
    // when they log back or create new account.
    xkreg::MempoolStatus::mempool_refresh_time = mempool_refresh_time;
    xkreg::MempoolStatus::start_mempool_status_thread();
 
    // create instance of page class which
    // contains logic for the website
    xkreg::page xkrblocks(&mcore,
                          core_storage,
                          daemon_url,
                          nettype,
                          enable_pusher,
    ...
    ([&](const crow::request& req) {
        return crow::response(xkrblocks.index2());
    });

    CROW_ROUTE(app, "/page/<uint>")
    ([&](size_t page_no) {
        return xkrblocks.index2(page_no);
    });

    CROW_ROUTE(app, "/block/<uint>")
    ([&](const crow::request& req, size_t block_height) {
        return crow::response(xkrblocks.show_block(block_height));
    });

    CROW_ROUTE(app, "/block/<string>")
    ([&](const crow::request& req, string block_hash) {
        return crow::response(xkrblocks.show_block(remove_bad_chars(block_hash)));
    });

    CROW_ROUTE(app, "/tx/<string>")
    ([&](const crow::request& req, string tx_hash) {
        return crow::response(xkrblocks.show_tx(remove_bad_chars(tx_hash)));
    });
    ...
    if (enable_as_hex)
    {
        ...
        ([&](string tx_hash) {
            return crow::response(xkrblocks.show_tx_hex(remove_bad_chars(tx_hash)));
        });

        CROW_ROUTE(app, "/ringmembershex/<string>")
        ([&](string tx_hash) {
            return crow::response(xkrblocks.show_ringmembers_hex(remove_bad_chars(tx_hash)));
        });

        CROW_ROUTE(app, "/blockhex/<uint>")
        ([&](size_t block_height) {
            return crow::response(xkrblocks.show_block_hex(block_height, false));
        });

        CROW_ROUTE(app, "/blockhexcomplete/<uint>")
        ([&](size_t block_height) {
            return crow::response(xkrblocks.show_block_hex(block_height, true));
        });
...
//        });
        ...
        CROW_ROUTE(app, "/ringmemberstxhex/<string>")
        ([&](string tx_hash) {
            return myxkr::jsonresponse {xkrblocks.show_ringmemberstx_jsonhex(remove_bad_chars(tx_hash))};
        });
    ...
    ([&](string tx_hash, uint16_t with_ring_signatures)
     {
        return xkrblocks.show_tx(remove_bad_chars(tx_hash), with_ring_signatures);
    });

    CROW_ROUTE(app, "/service_node/<string>")
    ([&](const crow::request& req, std::string service_node_pubkey) {
        return crow::response(xkrblocks.show_service_node(remove_bad_chars(service_node_pubkey)));
    });

    CROW_ROUTE(app, "/service_nodes")
    ([&](const crow::request& req) {
        return xkrblocks.render_service_nodes_html(true /*add_header_and_footer*/);
    });

    CROW_ROUTE(app, "/quorums")
    ([&](const crow::request& req) {
        return xkrblocks.render_quorum_states_html(true /*add_header_and_footer*/);
    });

    CROW_ROUTE(app, "/checkpoint_quorum/<uint>")
    ([&](const crow::request& req, uint64_t height) {
        return xkrblocks.render_single_quorum_html(service_nodes::quorum_type::checkpointing, height);
    });

    CROW_ROUTE(app, "/obligations_quorum/<uint>")
    ([&](const crow::request& req, uint64_t height) {
        return xkrblocks.render_single_quorum_html(service_nodes::quorum_type::obligations, height);
    });

    CROW_ROUTE(app, "/checkpoints")
    ([&](const crow::request& req) {
        return xkrblocks.render_checkpoints_html(true /*add_header_and_footer*/);
    });

    // TODO(kredits): This should be combined into the normal search mechanism, we shouldn't have 2 search bars.
    CROW_ROUTE(app, "/search_service_node").methods("GET"_method)
    ([&](const crow::request& req) {
        return xkrblocks.show_service_node(remove_bad_chars(string(req.url_params.get("value"))));
    });
    ...
    ([&](const crow::request& req)
     {
        ...
        map<std::string, std::string> post_body
                = xkreg::parse_crow_post_data(req.body);

        if (post_body.count("xkr_address") == 0
            || post_body.count("viewkey") == 0
            || post_body.count("tx_hash") == 0)
        {
            return string("xkr address, viewkey or tx hash not provided");
        }

        string tx_hash     = remove_bad_chars(post_body["tx_hash"]);
        string xkr_address = remove_bad_chars(post_body["xkr_address"]);
        ...
        return xkrblocks.show_my_outputs(tx_hash, xkr_address,
                                         viewkey, raw_tx_data,
                                         domain);
    });
    ...
    ([&](const crow::request& req, string tx_hash,
        string xkr_address, string viewkey)
     {

        string domain = get_domain(req);

        return xkrblocks.show_my_outputs(remove_bad_chars(tx_hash),
                                         remove_bad_chars(xkr_address),
    ...
    CROW_ROUTE(app, "/prove").methods("POST"_method)
        ([&](const crow::request& req) {
            ...
            if (post_body.count("xkraddress") == 0
                || post_body.count("txprvkey") == 0
                || post_body.count("txhash") == 0)
            {
                return string("xkr address, tx private key or "
            ...
            string xkr_address = remove_bad_chars(post_body["xkraddress"]);
            ...
            return xkrblocks.show_prove(tx_hash,
                                        xkr_address,
                                        tx_prv_key,
                                        raw_tx_data,
                                        domain);
    ...
    ([&](const crow::request& req, string tx_hash,
         string xkr_address, string tx_prv_key) {

        string domain = get_domain(req);

        return xkrblocks.show_prove(remove_bad_chars(tx_hash),
                                    remove_bad_chars(xkr_address),
    ...
    if (enable_pusher)
    {
        ...
        ([&](const crow::request& req) {
            ...
            if (action == "check")
                return xkrblocks.show_checkrawtx(raw_tx_data, action);
            else if (action == "push")
                return xkrblocks.show_pushrawtx(raw_tx_data, action);
    ...
    if (enable_key_image_checker)
    {
        ...
        ([&](const crow::request& req) {
            return xkrblocks.show_rawkeyimgs();
        });
        ...
        ([&](const crow::request& req) {
            ...
            return xkrblocks.show_checkrawkeyimgs(raw_data, viewkey);
        });
    }
    ...
    if (enable_output_key_checker)
    {
        ...
        ([&](const crow::request& req) {
            return xkrblocks.show_rawoutputkeys();
        });
        ...
        ([&](const crow::request& req) {
            ...
            return xkrblocks.show_checkcheckrawoutput(raw_data, viewkey);
        });
    }
    ...
    ([&](const crow::request& req) {
        return xkrblocks.search(remove_bad_chars(string(req.url_params.get("value"))));
    });

    CROW_ROUTE(app, "/mempool")
    ([&](const crow::request& req) {
        return xkrblocks.mempool(true);
    });
...
//    });
    ...
    if (enable_js)
    {
        ...
        ([&](const crow::request& req) {
            return xkrblocks.get_js_file("jquery.min.js");
        });

        CROW_ROUTE(app, "/js/crc32.js")
        ([&](const crow::request& req) {
            return xkrblocks.get_js_file("crc32.js");
        });

        CROW_ROUTE(app, "/js/biginteger.js")
        ([&](const crow::request& req) {
            return xkrblocks.get_js_file("biginteger.js");
        });

        CROW_ROUTE(app, "/js/crypto.js")
        ([&](const crow::request& req) {
            return xkrblocks.get_js_file("crypto.js");
        });

        CROW_ROUTE(app, "/js/config.js")
        ([&](const crow::request& req) {
            return xkrblocks.get_js_file("config.js");
        });

        CROW_ROUTE(app, "/js/nacl-fast-cn.js")
        ([&](const crow::request& req) {
            return xkrblocks.get_js_file("nacl-fast-cn.js");
        });

        CROW_ROUTE(app, "/js/base58.js")
        ([&](const crow::request& req) {
            return xkrblocks.get_js_file("base58.js");
        });

        CROW_ROUTE(app, "/js/cn_util.js")
        ([&](const crow::request& req) {
            return xkrblocks.get_js_file("cn_util.js");
        });

        CROW_ROUTE(app, "/js/sha3.js")
        ([&](const crow::request& req) {
            return xkrblocks.get_js_file("sha3.js");
        });
        ...
        ([&]() {
            ...
            return xkrblocks.get_js_file("all_in_one.js");
        });
    ...
    if (enable_json_api)
    {
        ...
        ([&](string tx_hash) {
            ...
            myxkr::jsonresponse r{xkrblocks.json_transaction(remove_bad_chars(tx_hash))};
        ...
        ([&](string tx_hash) {
            ...
            myxkr::jsonresponse r{xkrblocks.json_rawtransaction(remove_bad_chars(tx_hash))};
        ...
        ([&](string tx_hash) {
            ...
            myxkr::jsonresponse r{xkrblocks.json_detailedtransaction(remove_bad_chars(tx_hash))};
        ...
        ([&](string block_no_or_hash) {
            ...
            myxkr::jsonresponse r{xkrblocks.json_block(remove_bad_chars(block_no_or_hash))};
        ...
        ([&](string block_no_or_hash) {
            ...
            myxkr::jsonresponse r{xkrblocks.json_rawblock(remove_bad_chars(block_no_or_hash))};
        ...
        ([&](const crow::request &req) {
            ...
            myxkr::jsonresponse r{xkrblocks.json_transactions(
        ...
        ([&](const crow::request &req) {
            ...
            myxkr::jsonresponse r{xkrblocks.json_mempool(
                    remove_bad_chars(page), remove_bad_chars(limit))};
        ...
        ([&](string search_value) {
            ...
            myxkr::jsonresponse r{xkrblocks.json_search(remove_bad_chars(search_value))};
        ...
        ([&]() {
            ...
            myxkr::jsonresponse r{xkrblocks.json_networkinfo()};
        ...
        ([&]() {
            ...
            myxkr::jsonresponse r{xkrblocks.json_emission()};
            return r;
        });

        CROW_ROUTE(app, "/api/circulating_supply") ([&]() {
            std::string result = std::to_string(xkreg::CurrentBlockchainStatus::get_emission().coinbase / COIN);
        ...
        ([&](const crow::request &req) {
            ...
            myxkr::jsonresponse r{xkrblocks.json_outputs(
        ...
        ([&](const crow::request &req) {
            ...
            myxkr::jsonresponse r{xkrblocks.json_outputsblocks(
        ...
        ([&]() {
            ...
            myxkr::jsonresponse r{xkrblocks.json_version()};
    ...
    if (enable_autorefresh_option)
    {
        ...
        ([&]() {
            ...
            return xkrblocks.index2(page_no, refresh_page);
        });
    }
    ...
    if (enable_emission_monitor == true)
    {
        ...
        xkreg::CurrentBlockchainStatus::m_thread.interrupt();
        xkreg::CurrentBlockchainStatus::m_thread.join();
    ...
    xkreg::MempoolStatus::m_thread.interrupt();
    xkreg::MempoolStatus::m_thread.join();
