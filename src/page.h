...
#ifndef CROWXKR_PAGE_H
#define CROWXKR_PAGE_H
...
struct tx_details
{
    ...
    // public keys and xkr amount of outputs
    ...
    get_mstch_map() const
    {
        ...
        const double xkr_amount = XKR_AMOUNT(fee);
        ...
        {
            double payed_for_kB = xkr_amount / tx_size;

            mixin_str        = std::to_string(mixin_no);
            fee_str          = fmt::format("{:0.6f}", xkr_amount);
            fee_short_str    = fmt::format("{:0.4f}", xkr_amount);
            fee_micro_str    = fmt::format("{:04.0f}" , xkr_amount * 1e6);
        ...
        mstch::map txd_map {
                ...
                {"sum_inputs"        , xkr_amount_to_str(xkr_inputs , "{:0.6f}")},
                {"sum_outputs"       , xkr_amount_to_str(xkr_outputs, "{:0.6f}")},
                {"sum_inputs_short"  , xkr_amount_to_str(xkr_inputs , "{:0.3f}")},
                {"sum_outputs_short" , xkr_amount_to_str(xkr_outputs, "{:0.3f}")},
          ...
          tx_context_cache(1000)
{
    ...
    template_file["css_styles"]      = xkreg::read(TMPL_CSS_STYLES);
    template_file["header"]          = xkreg::read(TMPL_HEADER);
    template_file["footer"]          = get_footer();
    template_file["index2"]          = get_full_page(xkreg::read(TMPL_INDEX2));
    template_file["mempool"]         = xkreg::read(TMPL_MEMPOOL);
    template_file["altblocks"]       = get_full_page(xkreg::read(TMPL_ALTBLOCKS));
    template_file["mempool_error"]   = xkreg::read(TMPL_MEMPOOL_ERROR);
    template_file["mempool_full"]    = get_full_page(template_file["mempool"]);
    template_file["service_nodes"]   = xkreg::read(TMPL_SERVICE_NODES);
    template_file["quorum_states"]   = xkreg::read(TMPL_QUORUM_STATES);
    template_file["quorum_states_full"]  = get_full_page(template_file["quorum_states"]);
    template_file["checkpoints"]         = xkreg::read(TMPL_CHECKPOINTS);
    template_file["checkpoints_full"]    = get_full_page(template_file["checkpoints"]);
    template_file["service_nodes_full"]  = get_full_page(xkreg::read(TMPL_SERVICE_NODES));
    template_file["service_node_detail"] = get_full_page(xkreg::read(TMPL_SERVICE_NODE_DETAIL));
    template_file["block"]           = get_full_page(xkreg::read(TMPL_BLOCK));
    template_file["tx"]              = get_full_page(xkreg::read(TMPL_TX));
    template_file["my_outputs"]      = get_full_page(xkreg::read(TMPL_MY_OUTPUTS));
    template_file["rawtx"]           = get_full_page(xkreg::read(TMPL_MY_RAWTX));
    template_file["checkrawtx"]      = get_full_page(xkreg::read(TMPL_MY_CHECKRAWTX));
    template_file["pushrawtx"]       = get_full_page(xkreg::read(TMPL_MY_PUSHRAWTX));
    template_file["rawkeyimgs"]      = get_full_page(xkreg::read(TMPL_MY_RAWKEYIMGS));
    template_file["rawoutputkeys"]   = get_full_page(xkreg::read(TMPL_MY_RAWOUTPUTKEYS));
    template_file["checkrawkeyimgs"] = get_full_page(xkreg::read(TMPL_MY_CHECKRAWKEYIMGS));
    template_file["checkoutputkeys"] = get_full_page(xkreg::read(TMPL_MY_CHECKRAWOUTPUTKEYS));
    template_file["address"]         = get_full_page(xkreg::read(TMPL_ADDRESS));
    template_file["search_results"]  = get_full_page(xkreg::read(TMPL_SEARCH_RESULTS));

    partials["tx_details"]      = xkreg::read(string(TMPL_PARTIALS_DIR) + "/tx_details.html");
    partials["tx_table_head"]   = xkreg::read(string(TMPL_PARTIALS_DIR) + "/tx_table_header.html");
    partials["tx_table_row"]    = xkreg::read(string(TMPL_PARTIALS_DIR) + "/tx_table_row.html");
    partials["tx_type_symbol"]  = xkreg::read(string(TMPL_PARTIALS_DIR) + "/tx_type_symbol.html");

    if (enable_js) {
        // JavaScript files
        template_file["jquery.min.js"]   = xkreg::read(JS_JQUERY);
        template_file["crc32.js"]        = xkreg::read(JS_CRC32);
        template_file["crypto.js"]       = xkreg::read(JS_CRYPTO);
        template_file["cn_util.js"]      = xkreg::read(JS_CNUTIL);
        template_file["base58.js"]       = xkreg::read(JS_BASE58);
        template_file["nacl-fast-cn.js"] = xkreg::read(JS_NACLFAST);
        template_file["sha3.js"]         = xkreg::read(JS_SHA3);
        template_file["config.js"]       = xkreg::read(JS_CONFIG);
        template_file["biginteger.js"]   = xkreg::read(JS_BIGINT);
...
void add_tx_metadata(mstch::map &context, const cryptonote::transaction &tx, bool detailed = false) {
        ...
        if (new_style || get_service_node_state_change_from_tx_extra(tx.extra, state_change, cryptonote::network_version_11_infinite_staking)) {
           if (new_style) context["state_change_new_style"] = true;
            ...
            if (detailed) {
                ...
                // recent, this requires kreditsd to be started with --store-quorum-history (PR #702)
    ...
    else if (get_service_node_contributor_from_tx_extra(tx.extra, contributor))
    {
        ...
        context["contribution_amount"] = amount > 0 ? xkreg::xkr_amount_to_str(amount, "{:0.9f}", true) : "<decode error>";
    }
    else if (tx.version >= txversion::v4_tx_types && tx.type == txtype::kredits_name_system)
    {
      tx_extra_kredits_name_system lns_extra = {};
      if (get_kredits_name_system_from_tx_extra(tx.extra, lns_extra))
      {
...
index2(uint64_t page_no = 0, bool refresh_page = false)
{
    ...
    std::future<json> network_info_ftr = std::async(std::launch::async, [&]
    {
        ...
        get_kredits_network_info(j_info);
    ...
    mstch::map context {
            ...
            {"height"                   , height},
            {"server_timestamp"         , xkreg::timestamp_to_str_gm(local_copy_server_timestamp)},
    ...
    //double blk_size_median = xkreg::calc_median(blk_sizes.begin(), blk_sizes.end());
    ...
    context["network_info"] = mstch::map {
            {"difficulty"         , xkreg::make_comma_sep_number(current_network_info.difficulty)},
            {"hash_rate"          , hash_rate},
    ...
    if (CurrentBlockchainStatus::is_thread_running())
    {
        ...
        std::string emission_supply   = xkreg::make_comma_sep_number(current_values.circulating_supply / COIN);
        std::string emission_coinbase = xkreg::make_comma_sep_number(current_values.coinbase / COIN);
        std::string emission_fee      = xkreg::make_comma_sep_number(current_values.fee / COIN);
        std::string emission_burn     = current_values.burn ? xkreg::make_comma_sep_number(current_values.burn / COIN) : "0";
...
mempool(bool add_header_and_footer = false, uint64_t no_of_mempool_tx = 25)
{
    ...
    for (size_t i = 0; i < no_of_mempool_tx; ++i)
    {
        ...
        mstch::map context{
                ...
                {"xkr_inputs"      , mempool_tx.xkr_inputs_str},
                {"xkr_outputs"     , mempool_tx.xkr_outputs_str},
...
show_block(uint64_t _blk_height)
{
    ...
    string blk_timestamp = xkreg::timestamp_to_str_gm(blk.timestamp);
    ...
    mstch::map context {
            ...
            {"blk_difficulty"       , xkreg::make_comma_sep_number(blk_difficulty)},
            ...
            {"blk_cumulative_difficulty" , xkreg::make_comma_sep_number(blk_cumulative_difficulty)},
    };
    ...
    for (size_t i = 0; i < blk.tx_hashes.size(); ++i)
    {
        ...
        //string mixin_times_scale = xkreg::timestamps_time_scale(mixin_timestamps,
        //                                                        server_timestamp);
    ...
    context["sum_fees"]
            = xkreg::xkr_amount_to_str(sum_fees, "{:0.6f}", false);

    // get kredits in the block reward
    context["blk_reward"]
            = xkreg::xkr_amount_to_str(txd_coinbase.xkr_outputs - sum_fees, "{:0.6f}");
    ...
    if (!sn.funded) {
        ...
        for (const auto &mempool_tx : MempoolStatus::get_mempool_txs()) {
            ...
            if (get_service_node_contributor_from_tx_extra(mempool_tx.tx.extra, contributor)) {
                ...
                contrib["amount"] = amount > 0 ? xkreg::xkr_amount_to_str(amount, "{:0.9f}", true) : "<decode error>";
...
show_block(string _blk_hash)
{
    ...
    if (!xkreg::parse_str_secret_key(_blk_hash, blk_hash))
    {
...
show_tx(string tx_hash_str, uint16_t with_ring_signatures = 0)
{
    ...
    if (!mcore->get_tx(tx_hash, tx))
    {
        ...
        if (!found_txs.empty())
        {
            ...
            blk_timestamp = xkreg::timestamp_to_str_gm(tx_recieve_timestamp);
...
show_ringmembers_hex(string const& tx_hash_str)
{
    ...
    vector<txin_to_key> input_key_imgs = xkreg::get_key_images(tx);
                ...
                bool tx_prove = false)
{
    ...
    if (xkr_address_str.empty())
    {
        return string("Kredits address not provided!");
    }
    ...
    // parse string representing given kredits address
    cryptonote::address_parse_info address_info;

    if (!xkreg::parse_str_address(xkr_address_str,  address_info, nettype))
    {
        cerr << "Cant parse string address: " << xkr_address_str << endl;
        return string("Cant parse Kredits address: " + xkr_address_str);
    }
    ...
    if (!xkreg::parse_str_secret_key(viewkey_str, multiple_tx_secret_keys))
    {
...
//        if (!xkreg::parse_str_secret_key(spend_key_str, prv_spend_key))
//        {
...
//        }
    ...
    if (tx_blk_found)
    {
        ...
        blk_timestamp = xkreg::timestamp_to_str_gm(blk.timestamp);
    ...
    mstch::map context {
            ...
            {"xkr_address"          , xkr_address_str},
            {"viewkey"              , viewkey_str_partial},
            {"tx_pub_key"           , pod_to_hex(txd.pk)},
            {"blk_height"           , tx_blk_height_str},
            {"tx_size"              , fmt::format("{:0.4f}",
                                                  static_cast<double>(txd.size) / 1024.0)},
            {"tx_fee"               , xkreg::xkr_amount_to_str(txd.fee, "{:0.9f}", true)},
            {"blk_timestamp"        , blk_timestamp},
    ...
    string server_time_str = xkreg::timestamp_to_str_gm(server_timestamp, "%F");
    ...
    for (pair<txout_to_key, uint64_t>& outp: txd.output_pub_keys)
    {
        ...
        // if someone had sent us some xkr.
...
//             << pod_to_hex(tx_pubkey) << '\n'  << '\n';
        ...
        outputs.push_back(mstch::map {
                ...
                {"amount"                , xkreg::xkr_amount_to_str(outp.second)},
    ...
    // to hold sum of xkr in matched mixins, those that
    // perfectly match mixin public key with outputs in mixn_tx.
    uint64_t sum_mixin_xkr {0};
    ...
    // we get number of all posibilities, and their total xkr amount
    ...
    for (const txin_to_key& in_key: input_key_imgs)
    {
        ...
        inputs.push_back(mstch::map{
                ...
                {"key_image_amount", xkreg::xkr_amount_to_str(in_key.amount)},
        ...
        for (const uint64_t& abs_offset: absolute_offsets)
        {
            ...
            public_key mixin_tx_pub_key
                    = xkreg::get_tx_pub_key_from_received_outs(mixin_tx);
            ...
            output_pub_keys = xkreg::get_ouputs_tuple(mixin_tx);
...
//                        }
                ...
                found_outputs.push_back(mstch::map {
                        ...
                        {"amount"          , xkreg::xkr_amount_to_str(amount)}
                });
                ...
                if (mine_output)
                {
                    ...
                    // increase sum_mixin_xkr only when
...
//
//                        if (!xkreg::generate_key_image(derivation,
...
//                                << endl;
    ...
    context["found_our_outputs"] = (sum_xkr > 0);
    context["sum_xkr"]           = xkreg::xkr_amount_to_str(sum_xkr);

    context.emplace("inputs", inputs);

    context["show_inputs"]   = show_key_images;
    context["inputs_no"]     = static_cast<uint64_t>(inputs.size());
    context["sum_mixin_xkr"] = xkreg::xkr_amount_to_str(sum_mixin_xkr, "{:0.9f}", false);
    ...
    // useful for unit testing as it provides total xkr sum
    // of possible mixins
    ...
    if (sum_mixin_xkr > (sum_xkr + txd.fee)
        && no_of_matched_mixins == inputs.size())
    {
        //                  (outcoming    - incoming) - fee
        possible_spending = (sum_mixin_xkr - sum_xkr) - txd.fee;
    }

    context["possible_spending"] = xkreg::xkr_amount_to_str(
           ...
           string domain)
{
    ...
    return show_my_outputs(tx_hash_str, xkr_address_str,
...
show_checkrawtx(string raw_tx_data, string action)
{
    ...
    string data_prefix = xkreg::make_printable(decoded_raw_tx_data.substr(0, magiclen));
        ...
        if (r)
        {
            ...
            for (const ::tools::wallet2::tx_construction_data& tx_cd: exported_txs.txes)
            {
                ...
                mstch::map tx_cd_data {
                        ...
                        {"change_amount"      , xkreg::xkr_amount_to_str(tx_change.amount)},
                        {"has_payment_id"     , (payment_id  != null_hash)},
                        {"has_payment_id8"    , (payment_id8 != null_hash8)},
                        {"payment_id"         , pid_str},
                        {"payment_id8"        , pid8_str},
                ...
                for (const tx_destination_entry& a_dest: tx_cd.splitted_dsts)
                {
                    mstch::map dest_info {
                            ...
                            {"dest_amount"   , xkreg::xkr_amount_to_str(a_dest.amount)}
                    };
                ...
                for (size_t i = 0; i < no_of_sources; ++i)
                {
                    ...
                    mstch::map single_dest_source {
                            {"output_amount"              , xkreg::xkr_amount_to_str(tx_source.amount)},
                ...
                tx_cd_data.insert({"sum_outputs_amounts" ,
                                   xkreg::xkr_amount_to_str(sum_outputs_amounts)});
                ...
                tx_cd_data["min_mix_time"]     = xkreg::timestamp_to_str_gm(min_mix_timestamp);
                tx_cd_data["max_mix_time"]     = xkreg::timestamp_to_str_gm(max_mix_timestamp);
...
//                serialization::serialize(ba, tx_from_blob);
        ...
        for (tools::wallet2::pending_tx& ptx: ptxs)
        {
            ...
            if (ptx.construction_data.change_dts.amount > 0)
            {
                destination_addresses.push_back(
                        mstch::map {
                                ...
                                {"dest_amount"    ,
                                        xkreg::xkr_amount_to_str(ptx.construction_data.change_dts.amount)},
                                {"is_this_change" , true}
                        }
                );
            ...
            tx_context["outputs_xkr_sum"] = xkreg::xkr_amount_to_str(outputs_xkr_sum);
            ...
            for (size_t i = 0; i < outputs.size(); ++i)
            {
               ...
               uint64_t output_amount;
                ...
                if (parse_amount(output_amount, out_amount_str))
                {
                    if (output_amount == 0)
                    {
                        out_amount_str = xkreg::xkr_amount_to_str(real_ammounts.at(i));
                    }
                }
            }
            ...
            for (const tx_source_entry&  tx_source: ptx.construction_data.sources)
            {
                ...
                inputs_xkr_sum += tx_source.amount;
            }

            // mark that we have signed tx data for use in mstch
            tx_context["have_raw_tx"] = true;

            // provide total mount of inputs xkr
            tx_context["inputs_xkr_sum"] = xkreg::xkr_amount_to_str(inputs_xkr_sum);
            ...
            for (mstch::node& input_node: inputs)
            {
                ...
                amount = xkreg::xkr_amount_to_str(real_amounts.at(input_idx));
...
show_pushrawtx(string raw_tx_data, string action)
{
    ...
    // if failed, treat raw_tx_data as base64 encoding of signed_kredits_tx
    else
    {
        string decoded_raw_tx_data = epee::string_encoding::base64_decode(raw_tx_data);
...
show_checkrawkeyimgs(string raw_data, string viewkey_str)
{
    ...
    if (!xkreg::parse_str_secret_key(viewkey_str, prv_view_key))
    {
    ...
    string data_prefix = xkreg::make_printable(decoded_raw_data.substr(0, magiclen));
    ...
    decoded_raw_data = xkreg::decrypt(
    ...
    // get xkr address stored in this key image file
    const account_public_address* xkr_address =
            reinterpret_cast<const account_public_address*>(
                    decoded_raw_data.data());

    address_parse_info address_info {*xkr_address, false};


    context.insert({"address"        , REMOVE_HASH_BRACKETS(
            xkreg::print_address(address_info, nettype))});
    context.insert({"viewkey"        , REMOVE_HASH_BRACKETS(
            fmt::format("{:s}", prv_view_key))});
    context.insert({"has_total_xkr"  , false});
    context.insert({"total_xkr"      , string{}});
...
show_checkcheckrawoutput(string raw_data, string viewkey_str)
{
    ...
    address_parse_info address_info {*xkr_address, false, false, crypto::null_hash8};
    ...
    for (const tools::wallet2::transfer_details& td: outputs)
    {
        ...
        if (td.is_rct())
        {
            ...
            public_key tx_pub_key = xkreg::get_tx_pub_key_from_received_outs(tx);
        ...
        mstch::map output_info {
                ...
                {"amount"              , xkreg::xkr_amount_to_str(xkr_amount)},
                {"tx_hash"             , REMOVE_HASH_BRACKETS(fmt::format("{:s}", td.m_txid))},
                {"timestamp"           , xkreg::timestamp_to_str_gm(blk_timestamp)},
    ...
    if (total_xkr > 0)
    {
        context["has_total_xkr"] = true;
        context["total_xkr"] = xkreg::xkr_amount_to_str(total_xkr);
    }
...
search(string search_text)
{
    ...
    // check if kredits address is given based on its length
    ...
    if (search_str_length == 95)
    {
        ...
        if (!xkreg::parse_str_address(search_text, address_info, nettype_addr))
        {
    ...
    // check if integrated kredits address is given based on its length
...
show_address_details(const address_parse_info& address_info, cryptonote::network_type nettype = cryptonote::network_type::MAINNET)
{
    ...
    string address_str      = xkreg::print_address(address_info, nettype);
    ...
    mstch::map context {
            {"xkr_address"        , REMOVE_HASH_BRACKETS(address_str)},
    ...
    for (const pair<string, vector<string>>& found_txs: all_possible_tx_hashes)
    {
        ...
        if (!found_txs.second.empty())
        {
            ...
            for (const string& tx_hash: found_txs.second)
            {
                ...
                txd_map.insert({"timestamp", xkreg::timestamp_to_str_gm(blk_timestamp)});
...
json_transaction(string tx_hash_str)
{
    ...
    string blk_timestamp_utc = xkreg::timestamp_to_str_gm(tx_timestamp);
...
json_detailedtransaction(string tx_hash_str)
{
    ...
    tx_context.erase("inputs_xkr_sum_not_zero");
...
json_block(string block_no_or_hash)
{
    ...
    else if (block_no_or_hash.length() == 64)
    {
        ...
        if (!xkreg::parse_str_secret_key(block_no_or_hash, blk_hash))
        {
    ...
    j_data = json {
            ...
            {"timestamp_utc" , xkreg::timestamp_to_str_gm(blk.timestamp)},
            {"block_height"  , block_height},
            {"size"          , blk_size},
            {"txs"           , j_txs},
             ...
             bool tx_prove = false)
{
    ...
    {
        j_response["status"]  = "error";
        j_response["message"] = "Kredits address not provided";
        return j_response;
    }
    ...
    if (!xkreg::parse_str_address(address_str,  address_info, nettype))
    {
        j_response["status"]  = "error";
        j_response["message"] = "Cant parse Kredits address: " + address_str;
        return j_response;
    ...
    j_data["address"]  = REMOVE_HASH_BRACKETS(xkreg::print_address(address_info, nettype));
...
json_networkinfo()
{
    ...
    if (!get_kredits_network_info(j_info))
    {
        j_response["status"]  = "error";
        j_response["message"] = "Cant get Kredits network info";
        return j_response;
    }
...
json_version()
{
    ...
    j_data = json {
            ...
            {"kredits_version_full"   , string {KREDITS_VERSION_FULL}},
...
get_tx_json(const transaction& tx, const tx_details& txd)
{
    ...
    json j_tx {
            ...
            {"mixin"       , txd.mixin_no},
            {"tx_size"     , txd.size},
            {"xkr_outputs" , txd.xkr_outputs},
            {"xkr_inputs"  , txd.xkr_inputs},
...
construct_tx_context(transaction tx, uint16_t with_ring_signatures = 0)
{
    ...
    double payed_for_kB = XKR_AMOUNT(txd.fee) / tx_size;
    ...
    mstch::map context {
            ...
            {"tx_size"               , fmt::format("{:0.4f}", tx_size)},
            {"tx_fee"                , xkreg::xkr_amount_to_str(txd.fee, "{:0.9f}", false)},
            {"tx_fee_short"          , xkreg::xkr_amount_to_str(txd.fee, "{:0.4f}", false)},
            {"tx_fee_micro"          , xkreg::xkr_amount_to_str(txd.fee*1e6, "{:0.4f}", false)},
    ...
    for (const txin_to_key &in_key: txd.input_key_imgs)
    {
        ...
        inputs.push_back(mstch::map {
                ...
                {"amount"       , xkreg::xkr_amount_to_str(in_key.amount)},
        ...
        for (const uint64_t& i: absolute_offsets)
        {
            ...
            if (detailed_view)
            {
                ...
                mixins.push_back(mstch::map {
                        ...
                        {"mix_timestamp",  xkreg::timestamp_to_str_gm(blk.timestamp)},
    ...
    if (detailed_view)
    {
        ...
        context["min_mix_time"]     = xkreg::timestamp_to_str_gm(min_mix_timestamp);
        context["max_mix_time"]     = xkreg::timestamp_to_str_gm(max_mix_timestamp);
    ...
    context["inputs_xkr_sum_not_zero"]  = (inputs_xkr_sum > 0);
    context["inputs_xkr_sum"]           = xkreg::xkr_amount_to_str(inputs_xkr_sum);
    ...
    context["outputs_xkr_sum"] = xkreg::xkr_amount_to_str(outputs_xkr_sum);
...
)
{
    ...
    {
        // get mixins in time scale for visual representation
        pair<string, double> mixin_times_scale = xkreg::timestamps_time_scale(
                mixn_timestamps,
                max_mix_timestamp,
                170,
                min_mix_timestamp);
               ...
               uint64_t bc_height = 0)
{
    ...
    txd.pk = xkreg::get_tx_pub_key_from_received_outs(tx);
    txd.additional_pks = cryptonote::get_additional_tx_pub_keys_from_extra(tx);


    // sum xkr in inputs and ouputs in the given tx
    const array<uint64_t, 4>& sum_data = summary_of_in_out_rct(
            tx, txd.output_pub_keys, txd.input_key_imgs);

    txd.xkr_outputs       = sum_data[0];
    txd.xkr_inputs        = sum_data[1];
...
bool
get_kredits_network_info(json& j_info)
{
...
get_footer()
{
    ...
    string footer_html = mstch::render(xkreg::read(TMPL_FOOTER), footer_context);
