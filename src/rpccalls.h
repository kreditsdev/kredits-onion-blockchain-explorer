...
#ifndef CROWXKR_RPCCALLS_H
#define CROWXKR_RPCCALLS_H
...
{
// declare struct in kredits's cryptonote namespace.
// kredits should provide definition for this,
...
public:
    ...
    get_alt_blocks(vector<string>& alt_blocks_hashes)
    {
        ...
        if (r)
        {
            ...
            if (!err.empty())
            {
                cerr << "Error connecting to Kredits daemon due to "
                     << err << endl;
                return false;
            }
        }
        ...
        {
            cerr << "Error connecting to Kredits daemon at "
                 << daemon_url << endl;
            return false;
        }
