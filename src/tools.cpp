...
/**
* Parse kredits address in a string form into
...
/**
* Return string representation of kredits address
*/
string
...
{
    // default path to kredits folder
    // on linux this is /home/<username>/.kredits
    string default_kredits_dir = tools::get_default_data_dir();

    if (nettype == cryptonote::network_type::TESTNET)
        default_kredits_dir += "/testnet";
    if (nettype == cryptonote::network_type::STAGENET)
        default_kredits_dir += "/stagenet";


    // the default folder of the lmdb blockchain database
    // is therefore as follows
    return default_kredits_dir + string("/lmdb");
}
...
{
    // the default folder of the lmdb blockchain database
    string default_lmdb_dir   = xkreg::get_default_lmdb_folder(nettype);
    ...
    blockchain_path = xkreg::remove_trailing_path_separator(blockchain_path);
...
{
    pair<uint64_t, uint64_t> sum_xkr {0, 0};
    ...
    {
        sum_xkr.first += vout["amount"].get<uint64_t>();
        ++sum_xkr.second;
    }
...
{
    pair<uint64_t, uint64_t> sum_xkr {0ULL, 0ULL};
        ...
        vector<txin_to_key>& input_key_imgs)
{
    ...
    uint64_t xkr_outputs       {0};
    uint64_t xkr_inputs        {0};
    uint64_t mixin_no          {0};
    ...
    for (size_t i = 0; i < input_no; ++i)
    {
        ...
        xkr_inputs += tx_in_to_key.amount;
    ...
    return {xkr_outputs, xkr_inputs, mixin_no, num_nonrct_inputs};
};
...
summary_of_in_out_rct(const json& _json)
{
    ...
    for (const json& vout: _json["vout"])
    {
        xkr_outputs += vout["amount"].get<uint64_t>();
    }
    ...
    return {xkr_outputs, xkr_inputs, no_outputs, no_inputs, mixin_no, num_nonrct_inputs};
};
...
sum_money_in_inputs(const string& json_str)
{
    ...
    for (json& vin: j["vin"])
    {
        sum_xkr.first += vin["key"]["amount"].get<uint64_t>();
        ++sum_xkr.second;
    }
...
{
    array<uint64_t, 2> sum_xkr;

    sum_xkr[0] = sum_money_in_inputs(tx);
    sum_xkr[1] = sum_money_in_outputs(tx);
