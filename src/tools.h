...
#ifndef XKREG01_TOOLS_H
#define XKREG01_TOOLS_H
...
inline double
get_xkr(uint64_t core_amount)
{
    return XKR_AMOUNT(core_amount);
}
...
static
string
xkr_amount_to_str(const uint64_t& xkr_amount,
                  string _format="{:0.9f}",
                  bool zero_to_question_mark=true)
{
    ...
    {
        amount_str = fmt::format(_format, XKR_AMOUNT(xkr_amount));
    }
    else
    {
        if (xkr_amount > 0 && zero_to_question_mark == true)
        {
            amount_str = fmt::format(_format, XKR_AMOUNT(xkr_amount));
        }
    }
